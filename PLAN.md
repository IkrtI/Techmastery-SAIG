...
| 2026-07-16 | Mock data seeded on prod: scripts/seedMock.ts — 12 mock users (mock68xxxxx@kmitl.ac.th), 250 unique Thai posts across 14 days (seeded PRNG, per-mood phrase banks, raw insertMany preserves historical createdAt). Ran locally against a temporarily-exposed mongo port (Dokploy schedule exec 500s); port closed + schedule removed after. Cleanup: delete users by mock email prefix + their moods |
| 2026-07-16 | Faculty identity-lock: Faculty.code from official catalog (20 faculties incl. kosen/exchange/staff, full bachelor knownMajors); onboarding pins faculty from studentId digits 3-4, client select locked with hint; non-numeric SSO local part = staff — auto-onboarded to staff pseudo-faculty at callback, badge hides year. 51 server tests. Prod faculties re-seeded |
| 2026-07-18 | Security review pass: AgentShield project scan grade A (98); manual OWASP audit clean (0 npm vulns, no secret leaks, Zod everywhere, memory-held access token, httpOnly rotating refresh, anonymity tests). Hardening: shared middleware/rateLimits.ts — readLimiter 120/min on feed/stats/comments/faculties, authFlowLimiter 20/5min on OIDC login+callback, mutationLimiter moved out of moods.ts. Prod Mongo password rotated after temporary LAN exposure during seeding |
| 2026-07-18 | Security hardening round 2 (all review findings fixed): refresh-token family revocation — RefreshToken.family chain id, replay of a token revoked >30s = theft → whole family revoked + cookie cleared, ≤30s = concurrent-race loser (behavior unchanged); rate-limit keying — authed limiters bucket per user id, OIDC/refresh per client IP via CF-Connecting-IP (CF edge ≠ client behind tunnel); SEED_ADMIN_EMAILS now authoritative (unlisted admins downgraded at login); SSO_DEBUG logs claim keys only (no PII values); comment POST 400s at the 200/post visibility cap. SPECS §3/§4/§5/§6 synced. 51 server tests green |
